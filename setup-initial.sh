...
JENKINS_URL_IN_CONTAINER="http://${JENKINS_SERVICE_NAME}:${JENKINS_SERVICE_PORT}"
if [ -f /.dockerenv ]; then
  JENKINS_URL="${JENKINS_URL_IN_CONTAINER}"
else
...

{
  ...
  echo "JENKINS_URL_IN_CONTAINER=${JENKINS_URL_IN_CONTAINER}"
